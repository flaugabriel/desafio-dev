  ...
  def self.process_spreadsheet(path, user_id)
    ...
    File.open(path) do |file|
      file.each_line do |line|
        ...
        object_cnab = CnabCreator.create_cnab(object_hash[:type_cnabs].to_i, object_hash[:date_occurrence], new_total, object_hash[:card].to_i, object_hash[:hours], deal, store.id, user_id)
